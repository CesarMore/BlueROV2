...
class BlueRov(Bridge):
    ...
    def _create_pressure_msg(self):
    	if 'SCALED_PRESSURE2' not in self.get_data():
        	raise Exception('no SCALED_PRESSURE2 data')

    	pressure_data = self.get_data()['SCALED_PRESSURE2']

    	msg = FluidPressure()
    	self._create_header(msg)

    	# Set the pressure value in the message
    	msg.fluid_pressure = pressure_data['press_abs']

    	self.pub.set_data('/pressure', msg)
